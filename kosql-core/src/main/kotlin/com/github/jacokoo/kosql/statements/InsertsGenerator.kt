...
fun template(it: Int) = """
    ...
    |   infix fun<${times(it) {"T$it: Any"}}> VALUES(v: Value$it<${times(it) {"T$it"}}>): Repeat$it<${times(it) {"T$it"}}> = Repeat$it(append(InsertData(this.table, listOf(${times(it) {"this.c$it"}})), v))
    |}
...
fun classPart(it: Int) = """
    |data class Field$it<${times(it) { "T$it: Any" }}>(val table: Table<*>, ${times(it) { "val c$it: Column<T$it>" }}): ExtraValues {
